                        ...
                        <div class="row justify-content-center mb-3">
                            <div class="col-lg-8 col-md-10 col-xs-12 col-sm-12">
                                <div class="input-group input-group-lg">
                                    <input type="search" id="search" class="form-control rounded-pill px-3" placeholder="Find product by name or description">
                                    <span class="btn btn-outline border rounded-pill " id="search-icon" type="button"><i class="fa fa-search"></i></span>
                                </div>
                            </div>
                        </div>
                        <div class="row row-cols-xl-4 row-md-6 col-sm-12 col-xs-12 gy-3 gx-3 align-items-start justify-content-center">
                            <?php 
                                $qry = $conn->query("SELECT * FROM `product_list` where delete_flag = 0 and `status` = 1 order by `name` asc");
                                ...
                                <a class="card rounded-0 shadow product-item text-decoration-none text-reset" href="./?p=products/view_product&id=<?= $row['id'] ?>">
                                    <div class="position-relative">
                                        <div class="img-top position-relative product-img-holder">
                                            <img src="<?= validate_image($row['thumbnail_path']) ?>" alt="" class="product-img">
                                        </div>
                                    </div>
                                    <div class="card-body">
